import type { Algorithm, AlgorithmStep } from '../types';
import type { GraphState, GraphInput } from './types';

function buildAdjacencyList(input: GraphInput): Map<string, string[]> {
  const adj = new Map<string, string[]>();
  for (const node of input.nodes) {
    adj.set(node.id, []);
  }
  for (const edge of input.edges) {
    adj.get(edge.from)!.push(edge.to);
    if (!input.directed) {
      adj.get(edge.to)!.push(edge.from);
    }
  }
  // Sort neighbors alphabetically for deterministic traversal
  for (const [, neighbors] of adj) {
    neighbors.sort();
  }
  return adj;
}

function makeState(
  input: GraphInput,
  overrides: Partial<GraphState>,
): GraphState {
  return {
    nodes: input.nodes,
    edges: input.edges,
    visited: [],
    current: null,
    frontier: [],
    currentEdge: null,
    discoveryOrder: [],
    ...overrides,
  };
}

export const dfs: Algorithm<GraphState, GraphInput> = {
  meta: {
    name: 'Depth-First Search',
    slug: 'dfs',
    category: 'graph',
    complexity: {
      time: { best: 'O(V + E)', average: 'O(V + E)', worst: 'O(V + E)' },
      space: 'O(V)',
    },
    pseudocode: [
      'procedure DFS(G, start)',
      '  stack \u2190 [start]',
      '  visited \u2190 {}',
      '  while stack is not empty do',
      '    current \u2190 stack.pop()',
      '    if current in visited then continue',
      '    visited.add(current)',
      '    for each neighbor of current do',
      '      if neighbor not visited then',
      '        stack.push(neighbor)',
      '  return visited',
    ],
  },

  generateSteps(input: GraphInput): AlgorithmStep<GraphState>[] {
    const steps: AlgorithmStep<GraphState>[] = [];
    const adj = buildAdjacencyList(input);

    const visited: string[] = [];
    const frontier: string[] = [];
    const discoveryOrder: string[] = [];
    const startNode = input.nodes.find((n) => n.id === input.startNodeId)!;

    // Init step
    steps.push({
      state: makeState(input, {
        current: null,
        visited: [],
        frontier: [],
        currentEdge: null,
        discoveryOrder: [],
      }),
      activeLines: [0],
      description: `Starting DFS from node ${startNode.label}`,
      action: 'init',
    });

    // Push start node onto stack
    frontier.push(input.startNodeId);
    discoveryOrder.push(input.startNodeId);
    steps.push({
      state: makeState(input, {
        current: null,
        visited: [...visited],
        frontier: [...frontier],
        currentEdge: null,
        discoveryOrder: [...discoveryOrder],
      }),
      activeLines: [1],
      description: `Push start node ${startNode.label} onto stack`,
      action: 'push',
    });

    // Main DFS loop
    while (frontier.length > 0) {
      // Pop top of stack
      const currentId = frontier.pop()!;
      const currentNode = input.nodes.find((n) => n.id === currentId)!;

      // Check if already visited (skip)
      if (visited.includes(currentId)) {
        steps.push({
          state: makeState(input, {
            current: currentId,
            visited: [...visited],
            frontier: [...frontier],
            currentEdge: null,
            discoveryOrder: [...discoveryOrder],
          }),
          activeLines: [4, 5],
          description: `Pop ${currentNode.label} \u2014 already visited, skip`,
          action: 'pop',
        });
        continue;
      }

      // Pop step
      steps.push({
        state: makeState(input, {
          current: currentId,
          visited: [...visited],
          frontier: [...frontier],
          currentEdge: null,
          discoveryOrder: [...discoveryOrder],
        }),
        activeLines: [3, 4],
        description: `Pop ${currentNode.label} from the stack`,
        action: 'pop',
      });

      // Visit step
      visited.push(currentId);
      steps.push({
        state: makeState(input, {
          current: currentId,
          visited: [...visited],
          frontier: [...frontier],
          currentEdge: null,
          discoveryOrder: [...discoveryOrder],
        }),
        activeLines: [6],
        description: `Visit node ${currentNode.label}`,
        action: 'visit',
      });

      // Explore neighbors (reverse order so first alphabetical neighbor is on top of stack)
      const neighbors = adj.get(currentId) || [];
      const reversedNeighbors = [...neighbors].reverse();
      for (const neighborId of reversedNeighbors) {
        const neighborNode = input.nodes.find((n) => n.id === neighborId)!;

        if (!visited.includes(neighborId)) {
          // Push neighbor onto stack
          frontier.push(neighborId);
          if (!discoveryOrder.includes(neighborId)) {
            discoveryOrder.push(neighborId);
          }
          steps.push({
            state: makeState(input, {
              current: currentId,
              visited: [...visited],
              frontier: [...frontier],
              currentEdge: [currentId, neighborId],
              discoveryOrder: [...discoveryOrder],
            }),
            activeLines: [7, 8, 9],
            description: `Push neighbor ${neighborNode.label} onto stack (via edge ${currentNode.label}\u2192${neighborNode.label})`,
            action: 'push',
          });
        }
      }
    }

    // Done step
    steps.push({
      state: makeState(input, {
        current: null,
        visited: [...visited],
        frontier: [],
        currentEdge: null,
        discoveryOrder: [...discoveryOrder],
      }),
      activeLines: [10],
      description: `DFS complete! Visited ${visited.length} nodes in order: ${visited.join(' \u2192 ')}`,
      action: 'done',
    });

    return steps;
  },
};
